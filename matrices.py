...
# Pochodne funkcji ksztaltu (element 2D 4 wezly) po mu
n_ksi_prim = np.array([
    lambda mu : 0.25 * (mu - 1),
    lambda mu : 0.25 * (1 - mu),
    lambda mu : 0.25 * (1 + mu),
...
# Pochodne funkcji ksztalu (element 2D 4 wezly) po ksi
n_mu_prim = np.array([
...
# Wartosci pochodnych funkcji ksztaltu w punktach calkowania (2D, 4 wezly)
...
# Wagi w punktach calkowania (2D, 4 wezly)
...
# Dla obu schematow - macierze pochodnych po ksi oraz mu w kazdym punktow calkowania i wagi w tych punktach.
ELEMENT_4_2D = [
...
# Jakobiany elementu - dla kazdego punktu calkowania.
...
# Jakobian elementu - jeden (dla jednego punktu calkowania).
